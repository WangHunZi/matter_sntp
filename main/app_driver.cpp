...
static void app_driver_button_toggle_cb(void *arg, void *data)
{
    ...
    chip::DeviceLayer::SystemLayer().ScheduleLambda([] {
        static const char *tag_str = "sntp_callback";

        auto sntp_callback = [](uint64_t time, int, void *context) {
            uint16_t year;
            uint8_t  month, day, hour, minute, second;
            chip::SecondsSinceUnixEpochToCalendarTime(time, year, month, day, hour, minute, second);
        ...
        SNTPClientQuery("pool.ntp.org", sntp_callback, (void *) tag_str);
    });
}
